...
layui.define(['layer', 'laytpl', 'form', 'element', 'upload', 'util'], function(exports){
    ...
    var fly = {
        ...
        dir: layui.cache.domainUrl + '/lay/modules/fly/' //模块路径
        ...
        ,content: function(content){
            ...
            var html = function(end){
                return new RegExp('\\['+ (end||'') +'(b|pre|hr|div|span|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|lolita)([\\s\\S]*?)\\]', 'g');
            };
    ...
    $('#searchIndex').on('keyup', function (e) {
        if (e.keyCode === 13)
            location.href = layui.cache.domainUrl + '/search/index?keyword=' + $(this).val();
    })

    //签到
    var tplSignin = ['{{# if(d.signed){ }}'
        ,'<button class="layui-btn layui-btn-disabled">今日已签到</button>'
        ,'<span>获得了<cite>{{ d.experience }}</cite>Q豆</span>'
        ,'{{# } else { }}'
        ,'<button class="layui-btn layui-btn-danger" id="LAY_signin">今日签到</button>'
        ,'<span>可获得<cite>{{ d.experience }}</cite>Q豆</span>'
        ,'{{# } }}'].join('')
        ,tplSigninDay = '已连续签到<cite>{{ d.days }}</cite>天,最长连续签到<cite>{{ d.maxDays }}</cite>天'
        ...
        ,signRender = function(data){
            ...
            if(data.forgetDays.length === 0){
                $('#FQ_signdays').html('本月无漏签');
            }else {
                var signdaysHtml = '<table class="forget-days-table"><tr>'
                layui.each(data.forgetDays,function (index,item) {
                ...
                $('.forget-days-table td').on('click',function () {
                    var forgetDay = $(this).html();
                    layer.confirm('确定花5Q豆补签'+forgetDay+'号这天吗？', function(index){
                        fly.json('/sign/makeUp?forgetDay='+forgetDay,{},function (res) {
                            layer.msg('补偿成功！');
                            signRender(res.data);
                        });
                        layer.close(index);
    ...
    var e,t;
    $('body').on('mouseover','.fly-avatar',function () {
        var $avatar = $(this);
        var uid = parseInt($(this).attr('u-id'));
        if(!uid || uid <= 0||uid === layui.cache.user.userId) return
        e = setTimeout(function() {
            $.ajax({
                method: "GET",
                url: "/u/card/" + uid
            }).done(function(e) {
                var html = '<div id="u-card">'+e+'</div>'
                layer.tips(html, $avatar, {
                    tips: 3
                    ,time: 0
                    ,skin:'layer-names'
                });
                $('#u-card').hover(function () {
                    clearTimeout(t)
                },function () {
    ...
    }).on("mouseout", ".fly-avatar", function() {
        clearTimeout(e)
         t = setTimeout(function() {
            layer.closeAll("tips")
        }, 200)
    ...
    elemSigninHelp.on('click', function(){
        layer.open({
            ...
            ,content: ['<div class="layui-text" style="padding: 20px;">'
                ,'<blockquote class="layui-elem-quote">“签到”可获得社区Q豆，规则如下</blockquote>'
                ,'<table class="layui-table">'
                ,'<thead>'
                ,'<tr><th>连续签到天数</th><th>每天可获Q豆</th></tr>'
                ,'</thead>'
                ,'<tbody>'
                ...
                ,'<li>可补签，增加最长连续签到天数（只支持当月）</li>'
                ,'<li>排名改为以最长连续签到天数计算</li>'
                ,'<li style="color: #FF5722;">不可利用程序自动签到，否则Q豆清零</li>'
                ,'</ul>'
                ,'</div>'].join('')
        });
    });
    ...
    var tplSigninTop = ['{{# layui.each(d.data, function(index, item){ }}'
        ...
        ,'{{# } else { }}'
        ,'<span class="fly-grey">最长连续签到 <i>{{ item.maxDays }}</i> 天</span>'
        ,'{{# } }}'
        ,'</li>'
        ,'{{# }); }}'
        ,'{{# if(d.data.length === 0) { }}'
    ...
    var tplVisit = ['{{# layui.each(d.data, function(index, item){ }}'
        ,'<li>'
        ,'<a href="/u/{{item.visitUserId}}" target="_blank">'
        ,'<img src="{{item.visitorIcon}}">'
        ,'<cite class="c-fly-link">{{item.visitorName}}</cite>'
        ,'</a>'
        ,'{{# var date = new Date(item.visitTime); }}'
        ,'<span class="fly-grey"> {{ layui.laytpl.digit(date.getMonth()+1) + "月" + layui.laytpl.digit(date.getDate())+"日"+ layui.laytpl.digit(date.getHours())+"时" }}</span>'
        ,'</li>'
        ,'{{# }); }}'
        ,'{{# if(d.data.length === 0) { }}'
