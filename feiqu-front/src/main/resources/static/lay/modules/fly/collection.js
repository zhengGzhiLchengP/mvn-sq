...
layui.define(['fly','table','element','laypage'],function (exports) {
    ...
    if($('#doutuCollectioncard')[0]){
        fly.json('/collection/find/doutu', function(res){
            table.render({
                elem: '#doutuCollectioncard'
                ,data: res.data
                ,cols: [[
                    {field: 'content', title: '斗图描述', minWidth: 300, templet: '<div><a href="/fqDoutuCloud/{{d.topicId}}" target="_blank" class="layui-table-link">{{d.content}}</a></div>'
                    }
                    ,{field: 'collectTime', title: '收藏时间', width: 200, align: 'center', templet: '<div>{{d.collectTime}}</div>'
                    }
                ]]
