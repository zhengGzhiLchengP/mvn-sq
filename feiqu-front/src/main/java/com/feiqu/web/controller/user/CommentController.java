...
public class CommentController extends BaseController {
    ...
    */
    @GetMapping(value = "/my")
    public String myThoughtsPage(Model model){
        try {
            ...
            model.addAttribute(CommonConstant.SYSTEM_ERROR_CODE,"获取我的回复失败");
    ...
    @ResponseBody
    public Object list(@PathVariable Integer userId, @RequestParam(defaultValue = "1") Integer p){
...
//            PageInfo pageInfo = new PageInfo(comments);
            ...
            logger.error("comment list error",e);
            ...
            if(content.startsWith("@")){
                ...
                if(aiteUser != null){
                    if(!aiteUser.getId().equals(user.getId())){
                        String themeDes = "在<a class=\"c-fly-link\" href=\""+CommonConstant.DOMAIN_URL+ "/theme/"+comment.getTopicId()+   "\">帖子</a>中";
    ...
    public Object delete(Integer id, HttpServletRequest request, HttpServletResponse response){
        logger.info("评论 comment delete id:{}",id);
        BaseResult result = new BaseResult();
        try {
            FqUserCache user = webUtil.currentUser(request,response);
            if(user == null){
