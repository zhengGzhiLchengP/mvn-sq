package com.feiqu.quartz.spider;
...
import com.feiqu.system.mapper.mainData.FqTopicMapper;
import com.feiqu.system.mapper.mainData.FqTopicReplyMapper;
...
//    private static final Logger LOGGER = LoggerFactory.getLogger(TopicInfoPipeline.class);
    ...
    public void process(V2exDTO v2exDTO, Task task) {
        ...
        //当这段时间爬的数量大于50的时候 停止任务
        if(count >= 50){
            OOSpider ooSpider = (OOSpider)task;
            ooSpider.stop();
        }
        topicExample.clear();
        v2exDTO.setTitle(HtmlUtils.htmlEscape(v2exDTO.getTitle()));
        topicExample.createCriteria().andTitleEqualTo(v2exDTO.getTitle());
    ...
    private FqTopic DTO2DO(V2exDTO v2exDTO) {
        ...
        if(StringUtils.isEmpty(v2exDTO.getContent())){
            if(CollectionUtil.isEmpty(v2exDTO.getTopicContent())){
