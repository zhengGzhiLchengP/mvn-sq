...
//    private FqUserService userService;
    ...
    public FqUserCache currentUser(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
//            SecureUtil.des()
            cookieInfo = SecureUtil.des(secret.getBytes()).decryptStr(Base64.decode(userCookie));
        ...
        return CacheManager.getUserCacheByUid(Integer.parseInt(userId));
    }
...
//        request.setAttribute("user", user);
        ...
        String secret = CommonConstant.USER_COOKIE_SECRET;
        // 加密cookie
        String userCookie = SecureUtil.des(secret.getBytes()).encryptBase64(cookieBuilder.toString());
        String cookieKey = CommonConstant.USER_ID_COOKIE;
